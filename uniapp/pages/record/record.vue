<template>
	<view class="container">
		<v-tabs fixed v-model="current" class="tabs" :scroll="false" :tabs="tabs" lineHeight="4rpx" lineColor="#11ccc0" activeColor="#11ccc0" @change="changeTab"></v-tabs>
		<view class="orders" v-if="current == 0">
			<view>
				<view class="record">
		...
		<view class="orders" v-else="current == 1">
			<view>
				<view class="record">
					<view class="table-header">
	...
	export default {
		data() {
			return {
				...
				limit: 20,
				page: 1,
				tabs: [this._i18n.getLanguage().record.Lotteryrecord,this._i18n.getLanguage().record.bettingrecord],
			}
		},
		onShow() {
			if (this.current == 0) {
