define(['fast', 'template', 'moment'], function (Fast, Template, Moment) {
    ...
    function get_suggest() {
        $.ajax({
            type:"get",
            url:"/zxftNsHkXg.php/suggest/index?status=0&filter=%7B%22status%22%3A%220%22%7D&op=%7B%22status%22%3A%22%3D%22%7D",
            datatype:"json",
    ...
    function get_money() {
        $.ajax({
            type:"get",
            url:"/zxftNsHkXg.php/money/index?status=0&filter=%7B%22status%22%3A%220%22%7D&op=%7B%22status%22%3A%22%3D%22%7D",
            datatype:"json",
            success:function (res) {
                if (res.code!=0){
                    Backend.api.sidebar({
