...
class Admin extends Backend
{
    ...
    public function _initialize()
    {
        ...
        $this->childrenAdminIds = $this->getDataLimitAdminIds(true);
...
//            $users=\app\common\model\Users::where($where)->column('id');
     ...
     * @var array
     */
    protected $noNeedRight = ['aa'];
    function aa(){

        $adminModel=new \app\admin\model\Admin();
        dump($adminModel->fs_daili_arr(31));
        die;
        dump($adminModel->zc_daili_arr(32));die;
    }
...
//            ->where('admin_id', 'in', $childrenGroupIds)
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                //占成比例不能超过上级代理
                if ($params['zhancheng']>$this->model->getAdmin_zhancheng($params['admin_id'])){
                    $this->error(__("占成比例不能超过".$this->model->getAdmin_zhancheng($params['admin_id'])));
                }
...
//                    'email'    => 'require|email|unique:admin,email,' . $row->id,
                    'password' => 'regex:\S{32}',
                    'zhancheng'=>'require|max:100|min:0',
                    'rebate'=>'require|max:100|min:0',
                ]);
